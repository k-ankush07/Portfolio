    ...
    <section className="container">
      <div className="px-[20px] md:px-0 pb-[48px] pt-[32px] lg:pt-0">
          ...
          {ServicesArray.map((item) => {
            ...
            return (
              <div
                ...
                className={`py-[16px] lg:py-[12px] px-[20px] border-[1px] border-[#FFFFFF1A]/10 rounded-[5px] cursor-pointer transition-all duration-200 ${
                  isSelected ? "bg-white text-black" : "bg-[#232323] text-white"
                }`}
              >
                <p className="text-[12px] lg:text-[18px]">{item.Name}</p>
              </div>
            )
          })}
        </div>
        ...
        <div className="pt-[15px] lg:pt-[20px]">
          ...
          <div className="flex flex-wrap gap-[10px]">
            {ArtArray.map((item) => {
              const isSelected = selectedNiches.includes(item.topic)
              return (
                <div
                  key={item.id}
                  onClick={() => onToggleNiche(item.topic)}
                  className={`py-[12px] px-[19px] rounded-[5px] cursor-pointer transition-all duration-200 ${
                    isSelected ? "bg-white text-black" : "bg-[#232323] text-white"
                  }`}
                >
                  <p className="text-[12px] lg:text-[18px]">{item.topic}</p>
                </div>
              )
            })}
          </div>
